//! Copies `.cargo/config.toml.template` to `.cargo/config.toml` for the `va416xx` workspace on
...
//! regular dependency rather than built from within the `va416xx` workspace) or the copy fails
